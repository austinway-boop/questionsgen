...
COURSE_TREES = {
    "APHG": {"name": "AP Human Geography", "mmd": "skill_tree.mmd"},
    "APGOV": {"name": "AP Government", "mmd": "data/apgov-skill-tree.mmd"},
    "APUSH": {"name": "AP US History", "mmd": "data/apush-skill-tree.mmd"},
    "APWH": {"name": "AP World History", "mmd": "data/apwh-skill-tree.mmd"},
}


def _get_course_tree(course_id):
    info = COURSE_TREES.get(course_id)
    if not info:
        return None
    return parse_skill_tree(Path(__file__).resolve().parent / info["mmd"])


@app.route("/courses")
def list_courses():
    result = {}
    for cid, info in COURSE_TREES.items():
        tree = _get_course_tree(cid)
        total_skills = sum(len(u["skills"]) for u in tree["units"]) if tree else 0
        result[cid] = {"name": info["name"], "units": len(tree["units"]) if tree else 0, "skills": total_skills}
    return jsonify(result)


@app.route("/graph-data")
def graph_data():
    """Return mermaid graph for one or all courses."""
    course = request.args.get("course", "")
    compact = request.args.get("compact", "0") == "1"

    if course and course in COURSE_TREES:
        courses = {course: COURSE_TREES[course]}
    else:
        courses = COURSE_TREES
    ...
    for cid, cinfo in courses.items():
        ...
        if not tree:
            continue
        prefix = cid + "_" if len(courses) > 1 else ""
        course_label = cinfo["name"]

        for unit in tree["units"]:
            uid = prefix + unit["id"]
            title = f"{course_label} — {unit['title']}" if len(courses) > 1 else unit["title"]
            unit_skills[uid] = {"title": title, "skills": [], "course": cid}
            lines.append(f'    subgraph {uid}["{title}"]')
            lines.append(f"    direction LR")
            for skill in unit["skills"]:
                sid = prefix + skill["id"]
                num = skill["id"].replace(unit["id"], "S")
                label = num if compact else skill["id"]
                lines.append(f"    {sid}(({label}))" if compact else f'    {sid}["{label}"]')
                unit_skills[uid]["skills"].append(sid)
            for edge in unit["edges"]:
                lines.append(f"    {prefix}{edge[0]} --> {prefix}{edge[1]}")
            lines.append("    end")
            unit_counter += 1
    ...
    # Assign color classes based on unit index
    for uid, data in unit_skills.items():
        class_name = uid.split("_")[-1].lower() if "_" in uid else uid.lower()
